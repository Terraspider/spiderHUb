...
*  dd/mm/yyyy   	1.0.0   	Shibin,Suraj
*
*******************************************************************************/
/** @file      :	Application.c
 *  Description:	Middleware between main program and end device protocols.
 *			Processes the data.
*/
/******************************************************************************
 * Includes
 ...
 switch(validate_req_type(inclusion, dev_type))   		 {
  case inclusion :
	 if ((strcmp(cmd_id,"ADD") == 0) && (net_status))	 {
	     ...
	     system("/SpiderHub/Scripts/bluetooth_voice_alert.sh Spider, is ready to add device");
	     online_access = false;	 						 }
	  break;
  case exclusion :
 	 if ((strcmp(cmd_id,"DEL") == 0) && (net_status)) 	 {
 	     printf("Success : BLE Device to DELETE\n");
	     system("/SpiderHub/Scripts/bluetooth_voice_alert.sh Spider, is ready to delete device");
 	     online_access = false; 	 					 }
 	  break;
    }
  if(!net_status){
   err_val = Gw_Auto_val_response("INVALID : Do not Support in Offline");
   system("/SpiderHub/Scripts/bluetooth_voice_alert.sh Add or delete feature, not supported Offline");	}
  ...
  if (status == bluetooth_proto)
  {
   ...
   switch(validate_req_type(audio, dev_type))
   {
    case audio :
	   ...
	   {
		   system("/SpiderHub/Scripts/start_BLE.sh");
		   sleep(2);
		   printf("Success : BLUETOOTH Device Switched ON\n");
		   device_config_response("AUDIO ENABLE", proto_id);
		   system("/SpiderHub/Scripts/bluetooth_voice_alert.sh");
		   enable_ble = 'Y';
	   }
	   else if ((strcmp(cmd_id,"DISABLE") == 0))
	   {
		   system("/SpiderHub/Scripts/stop_BLE.sh");
		   sleep(2);
...
/* ========================== End of BLE ===============================*/
 ...
 else if (status == cam_proto)
 {
    ...
    case search :
     ...
     system("/SpiderHub/Scripts/get-onvif-cameras.sh");
     char cam_name[1000];
     FILE *cam = fopen("/SpiderHub/Libraries/ws-discovery/WSDiscovery-0.2/onvif-cameras-list.txt","r");
         if (cam == NULL)
         {
    ...
    // char cam_motion[200];
	 ...
	 strcpy(cam_motion,"/SpiderHub/Scripts/Camera_motion.sh ");
	 ...
	 system("/SpiderHub/Scripts/bluetooth_voice_alert.sh Enabling camera controls. This will take 1 minute. Please wait");
    break;

    case up :
	 printf("Success : Camera is moving up ^^\n");
	 //printf("camera direction::::::::: %s\r\n",dev_type);
     system("echo up > /SpiderHub/Libraries/onvif/python-onvif/examples/Direction_file.txt");
     err_val = false;
    break;
    case down :
	 printf("Success : Camera is moving down\n");
     system("echo dn > /SpiderHub/Libraries/onvif/python-onvif/examples/Direction_file.txt");
     err_val = false;
    break;
    case left :
	 printf("Success : Camera is moving left <<\n");
     system("echo lt > /SpiderHub/Libraries/onvif/python-onvif/examples/Direction_file.txt");
     err_val = false;
    break;
    case right :
     printf("Success : Camera is moving right >>\n");
     system("echo rt > /SpiderHub/Libraries/onvif/python-onvif/examples/Direction_file.txt");
     err_val = false;
    break;
    case stop :
