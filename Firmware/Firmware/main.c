...
char Mqtt_connect(int internet_stat)
  {
	...
	char willTopic[50];
	//int rc;
	FILE *fileptr = fopen("/SpiderHub/Flags/internet_status.txt","w");
	int ch;
	if(internet_stat == 0)	{
	  MQTTClient_create(&client, OFFLINE_ADDRESS, CLIENTID,
	  MQTTCLIENT_PERSISTENCE_NONE, NULL);
	  printf("Success : Gateway_OFFLINE\n");
	  system("/SpiderHub/Scripts/bluetooth_voice_alert.sh I lost Internet: use me off line");
	  off_line_flag = true;
	  ...
	  system("/SpiderHub/Scripts/bluetooth_voice_alert.sh I am going online");
	  sleep(1);
	  system("kill $(ps aux | grep 'filereceive.py' | awk '{print $2}')");
	  system("python /SpiderHub/Scripts/filereceive.py > /dev/null 2>&1 &");
	  inter_stat = 0;
	  off_line_flag = false;
	...
	if ((rc = MQTTClient_connect(client, &conn_opts)) != MQTTCLIENT_SUCCESS)	{
	  ...
	  system("/SpiderHub/Scripts/bluetooth_voice_alert.sh Something wrong I am unable to contact server");
	  mqtt_status_flag = 'F';
...
void main()
{
	...
	printf("*********** %s ************\n", version);
        FILE *ver = fopen("/SpiderHub/Version/versions.txt","w");
        fprintf(ver,"%s",version);
	fclose(ver);
	//system("./ota_update.sh > /dev/null 2>&1 &");
	if( access("/SpiderHub/UserID_Database/registered_user_ids.txt", F_OK ) != -1 )  {
    	printf("Registered_user_ids DB file found!!!\n"); }
	else {
	 FILE *fptr = fopen("/SpiderHub/UserID_Database/registered_user_ids.txt","w");
	 if (fptr == NULL)	{
	...
	if ((off_line_flag == true) && (t >= time_now + 30))	{
	    time_now = t;
		inter_stat = system("/SpiderHub/Scripts/Internet_connection.sh");
		printf("Success : Internet_status : %d\n", inter_stat);
	  if(inter_stat == 256){
		Gw_Auto_val_response("GATEWAY ONLINE");
		system("/SpiderHub/Scripts/bluetooth_voice_alert.sh I got internet");
		Online_status_publish();	}	}
